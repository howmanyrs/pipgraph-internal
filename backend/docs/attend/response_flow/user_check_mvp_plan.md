...
См. [state_serialization_details.md](./state_serialization_details.md) для подробного объяснения:
- Почему PipGraphManager не сериализуется
- Как работает custom serializer (FeedbackStateSerializer)
- Manual injection при resume
...
- [ ] Create `NoteProcessingState` TypedDict с `total=False`
- [ ] Create `FeedbackStateSerializer` (custom serializer)
- [ ] Implement simple 2-node graph (extract → finalize)
- [ ] Setup AsyncSqliteSaver с custom serializer
- [ ] Test basic graph execution with state persistence
- [ ] Test manager injection при resume
...
- `backend/app/services/feedback_state_serializer.py` - Custom serializer (см. [state_serialization_details.md](./state_serialization_details.md))
...
def create_feedback_graph(checkpointer_path: str = "sessions.db"):
    ...
    # Compile with checkpointer and custom serializer
    # Note: Custom serializer исключает pipgraph_manager из сериализации
    # См. state_serialization_details.md для деталей
...
async def websocket_feedback_endpoint(
    ...
    try:
        ...
        if state_snapshot and state_snapshot.next:
            ...
            # CRITICAL: Inject PipGraphManager into loaded state
            # (Custom serializer excluded it during save)
            # См. state_serialization_details.md для деталей
            current_state = state_snapshot.values
            current_state['pipgraph_manager'] = pipgraph_manager  # ← Manual injection
            if current_state.get('current_clarification'):

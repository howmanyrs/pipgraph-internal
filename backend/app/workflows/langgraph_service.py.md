...
def create_para_workflow(checkpointer=None):
    ...
    workflow.add_node("identify_context", para_workflow.identify_context_node)
    workflow.add_node("apply_proposal", para_workflow.apply_proposal_node)
    workflow.add_node("wait_for_decision", para_workflow.wait_for_decision_node)
    workflow.add_node("process_decision", para_workflow.process_decision_node)
    workflow.add_node("extract_content", para_workflow.extract_content_node)
    workflow.add_node("save_entities", para_workflow.save_entities_node)
...
async def get_workflow_status(thread_id: str) -> dict:
    """
    Get current workflow state by thread_id.

    Convenience wrapper that auto-resolves the compiled workflow.
    Used by endpoints that only have thread_id.

    Args:
        thread_id: Thread ID (e.g., "note:meetings/sync.md")

    Returns:
        dict with current state or empty dict if not found
    """
    config = {"configurable": {"thread_id": thread_id}}

...
async def identify_context_node(state: PARAWorkflowState) -> dict:
    ...
    try:
        ...
        para_type = para_service.classify_note_para(note_content)
        ...
        """
        proposal = para_service.generate_para_proposal(note_content)
        logger.info(
        ...
        serialized_proposal = workflow_state.serialize_proposal(proposal)
...
async def apply_proposal_node(state: PARAWorkflowState) -> dict:
    ...
    try:
        ...
        result = proposal_manager.apply_proposal_to_graph(
...
async def process_decision_node(state: PARAWorkflowState) -> dict:
    ...
    try:
        ...
        if user_decision.action == "confirm" and result.get("details", {}).get("type") == "link":
            cascade_svc = cascade_service.CascadeService()
            cascade_response = cascade_svc.process_decision_with_cascade(
...
async def extract_content_node(state: PARAWorkflowState) -> dict:
    ...
    try:
        ...
        serialized_entities = workflow_state.serialize_entities(entities)
...
async def save_entities_node(state: PARAWorkflowState) -> dict:
    ...
    try:
        ...
        entities = workflow_state.deserialize_entities(extracted_entities_data)

        # Save entities and create :MENTIONS relationships
        crud = entity_crud.EntityCRUD()
        result = crud.batch_save_entities(
            entities=entities,
            episodic_path=note_path
        )

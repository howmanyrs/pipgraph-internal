...
| DELETE | `/dev/graph` | **Debug only.** Wipe the entire graph in one `MATCH (n) DETACH DELETE n` — every node and edge. Backs the Obsidian plugin's "Danger zone" reset. Irreversible; no soft-delete. Returns `deleted_nodes_count`. |
...
- `finalize_episode_name(uuid, name)` — set final `name` + clear `status` (naming job, LLM-success branch). `set_episode_name(uuid, name)` — set `name` but **keep** `status` (naming job, fallback branch: stores the text-derived name yet leaves `status="failed:generate_episode_name"` so the client shows a ❗ + "regenerate"). `set_episodic_status(uuid, status)` — set the transient `status` (e.g. `failed:<job>`). `clear_episodic_status(uuid)` — remove `status` on success of a job that doesn't change `name` (the `process_existing_episode` job).

Вот обновленная версия документа `03_PIPGRAPH_MANAGER_REFACTORING.md`.

**Ключевые изменения относительно версии 1.0:**
1.  **Top-Down архитектура:** Методы сгруппированы так, чтобы сначала решать судьбу заметки (PARA), и только потом извлекать сущности.
2.  **No-Cache Policy:** Удалены методы обновления атрибутов (`update_note_para_type`). Вместо этого добавлены методы для эффективного чтения связей (Graph Traversal).
3.  **Context-Aware Extraction:** Метод извлечения сущностей теперь принимает контекст проекта и использует жесткую схему типов, чтобы избежать "мусора" от Graphiti.

---
--- START OF FILE 03_PIPGRAPH_MANAGER_REFACTORING.md ---

# PipGraphManager Refactoring: Architecture Analysis (v2.0)
...
В версии 2.0 мы отказываемся от попытки "сделать всё сразу" и от хранения избыточных данных.
...
## 3. Сравнение: Старый (v1) vs Новый (v2) подход

| Характеристика | v1 (Legacy) | v2 (Simplified) |
| :--- | :--- | :--- |
...
**Следующий документ:** [04_LANGGRAPH_WORKFLOW_v03.md](./04_LANGGRAPH_WORKFLOW_v03.md)
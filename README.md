...
5.  Бэкенд запускает обработку: классифицирует заметку, извлекает сущности с помощью LLM и сохраняет их в графовой базе данных. Для прототипа это прямой вызов, в будущем — фоновая задача.
6.  После завершения обработки бэкенд отправляет по тому же WebSocket-соединению финальное сообщение с извлеченными данными.
7.  Плагин, получив финальное сообщение, обновляет YAML-секцию в файле заметки и закрывает соединение.

...
> **Status: iterative build.** A working plugin runs in a test vault — settings (incl. runtime LLM-provider + extraction-prompt editing), inbox capture, a vault↔graph folder mirror, a tabbed triage panel, drag-to-place, and **suggestion-driven placement** (ranked PARA-folder recommendations the user confirms by hand). What's still taking shape is the *automatic* end of triage — auto-placement, frontmatter enrichment, and a reporting dashboard. This file describes *where this component sits*, *what it is for*, and *what surrounds it* — the slow-changing frame. Concrete, churning decisions (APIs, UI patterns, file layouts) live under [`.docs/suggestions/`](./.docs/suggestions) (design) and [`.docs/plans/`](./.docs/plans) (how it ships); *what actually stands week-to-week* lives there and in [`JOURNAL.md`](./JOURNAL.md), not here.
...
- **Shipped, exercised in a vault:** a typed HTTP client over the backend; a settings tab (backend URL, PARA root folder, inbox/drafts subfolders; an **LLM provider** section — pick Cloud.ru/OpenRouter + keys/models, pushed to the backend via `/dev/llm-config`, applied on backend restart; and a **Prompts** section that edits the domain guidance of the backend's extraction prompts via `/dev/prompts`, applied live); inbox-capture commands (quick-capture modal, draft note, process-a-draft); a **folder ↔ entity mirror** — PARA folders under the root are reflected as graph entities, the filesystem hierarchy becomes `BELONGS_TO`, and deleting a folder cascades to its orphaned notes (folders with no summary get a small file-explorer marker); a **tabbed triage panel** whose *Entity Inspector* shows a clicked folder's graph node with an editable summary; **drag-to-place** — dragging a note from the inbox tab onto a PARA folder moves the file and links it (`MENTIONS`) in one gesture; **suggestion-driven placement** — a *Focus suggest* toggle ranks PARA folders for the selected note and surfaces them either as %-match badges on the real explorer tree or as a "ghost tree" of candidates, each confirmable by click/right-click/drag; **status markers** in the explorer (`⟳` processing · `⚠` failed → right-click *Process note* / the *Process all failed notes* command · `❗` auto-named → *Regenerate name with LLM*); and a settings **Danger zone** with two confirmed debug resets — *wipe graph* (`DELETE /dev/graph`, every node + edge) and *clear vault folder* (delete notes + emptied PARA subfolders under the root, plus pending capture files).
- **In flight / next:** **automatic placement** on capture (the manual recommendation flow above already works; auto-assignment is being tuned); **frontmatter enrichment** — writing entity-derived tags/links into a note's YAML, body untouched; and a **reporting dashboard** over the graph's entity↔note links + timestamps (active topics, abandoned-topic reminders, rollup summaries). Smaller open items: a tentative/pending decoration distinguishing *auto-suggested* from *human-placed* notes, and backend gaps the plugin surfaced (e.g. editing an entity's name / path — summary already landed).
